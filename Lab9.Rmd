...
- Cross-validation in Machine Learning. Package: `carat`, `mlr`
- Bootstrapping. Package: `boot`
- Monte Carlo Simulation. Package: `parallel`
...
library(parallel)
myboot <- function(dat, stat, R, ncpus = 1L) {
...
system.time(myboot(dat = data.frame(x, y), my_stat, R = 3000, ncpus = 1L))
system.time(myboot(dat = data.frame(x, y), my_stat, R = 3000, ncpus = 2))
```
...
```
```bash
Rscript --vanilla -e 'rmarkdown::render("~/tsclient\mariabolshakova\Documents\Documents - Maria’s MacBook Air\Year 4\PM 566 Health Data Science\PM566-labs")'
```
